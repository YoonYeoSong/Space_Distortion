...
public class AdminView extends SpaceActionEvent
		implements ViewIndex, TableCellRenderer {
	...
	public void initialize(SpaceController sc, JFrame mainJframe) {
		...
		btn3Snack.setBounds(btn2RoomInfo.getX() + 200, btn1Member.getY(), btn1Member.getWidth(),
				btn1Member.getHeight());
		...
		// 페이먼트 버튼
		btn5TotalPay = new JButton(); // setBounds(btn3Snack.getX()+200, btn1Member.getY(), btn1Member.getWidth(),
										// btn1Member.getHeight());
		btn5TotalPay.setBounds(btn3Snack.getX() + 200, btn1Member.getY(), btn1Member.getWidth(),
				btn1Member.getHeight());
		...
		// 예약 버튼
		btn4Reservation = new JButton(); // setBounds(btn4Reservation.getX()+200, btn1Member.getY(),
											// btn1Member.getWidth(), btn1Member.getHeight());
		btn4Reservation.setBounds(btn5TotalPay.getX() + 200, btn1Member.getY(), btn1Member.getWidth(),
				btn1Member.getHeight());
...
//				tableBackLabel.revalidate();
//				tableBackLabel.repaint();
				int ans = JOptionPane.showConfirmDialog(null, "List들을 추가 하시겠습니까?", "확인", JOptionPane.YES_NO_OPTION,
		...
		btn6Delete.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (table.getSelectedRow() == -1) {
					...
					// 삭제할지 말지 다이얼로그로 판별
					int ans = JOptionPane.showConfirmDialog(null, "정말로 삭제 하시겠습니까?", "확인", JOptionPane.YES_NO_OPTION,
							JOptionPane.PLAIN_MESSAGE);
					// true 0
					if (ans == 0) {
						// 칼럼의 수로 스낵 판별 5
		...
		btnCoupon.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (table.getSelectedRow() == -1) {
					...
					int ans = JOptionPane.showConfirmDialog(null, "쿠폰을 발행 하시겠습니까?", "확인", JOptionPane.YES_NO_OPTION,
							JOptionPane.PLAIN_MESSAGE);
					...
					if (ans == 0) {
						...
						// 칼럼수 9 회원
							...
							String[] modelName = { "회원코드", "이름", "이메일", "비밀번호", "주소", "핸드폰 번호", "생년월일", "학생이면 1",
									"쿠폰" };
							defaultModel = new DefaultTableModel(spaceController.adminSearchAllMember(modelName),
									modelName) {
								@Override
								public boolean isCellEditable(int row, int column) {
