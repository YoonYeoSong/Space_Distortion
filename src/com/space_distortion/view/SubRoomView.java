...
//	private JLabel lb7;
	...
	public void initialize(SpaceController sc, JFrame mainJframe,List roomInfoList, int roomNumber)
	{
		...
		btnBack.setBounds(331, 25, 138, 27);
		jp.add(btnBack);
		

		btnMember = new JButton("회원");
		btnMember.setBounds(35, 414, 105, 27);
		jp.add(btnMember);
		
		btnNoMember = new JButton("비회원");
		btnNoMember.setBounds(198, 414, 105, 27);
		jp.add(btnNoMember);
		
		btnReservation = new JButton("예약");
		btnReservation.setBounds(361, 414, 105, 27);
		jp.add(btnReservation);
		
		lb1.setText(   ((RoomInfo)roomInfoList.get(roomNumber-1)).getRoomName());
		lb2.setText("방번호 :  " + String.valueOf(   ((RoomInfo)roomInfoList.get(roomNumber-1)).getRoomNumber())); //스트링으러 바꿈
